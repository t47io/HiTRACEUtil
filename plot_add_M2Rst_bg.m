function plot_add_M2Rst_bg(d_align, label, helix)
temp = strsplit(regexprep(label, '[ACGTU]', ''), ';');
...
for i = 1:length(temp);
    rectangle('Position',[str2num(temp{i})-0.5 4.5 1 0.5], 'FaceColor',[1, 0.36, 0.17], 'EdgeColor','none');hold on;
end;

image(-25:152,1:4, d_align'*60); colormap(1-gray);
...
h = title(label, 'fontsize',12,'fontweight','bold');
p = get(h,'Position');
